...
class StreamGearbox(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        with m.If(input_read & ~output_write):
            ...
            m.d.sync += shift_register.eq(Cat(shift_register[:-input_width], self.input.payload))
        ...
        with m.Elif(input_read & output_write):
            m.d.sync += shift_register.eq(Cat(shift_register[:-output_width], self.input.payload))
