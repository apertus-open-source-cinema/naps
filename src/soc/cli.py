...
def cli(top_class, runs_on, possible_socs=(None,)):
    ...
    hardware_platform = getattr(__import__('devices'), "{}Platform".format(args.device))
    if args.soc:
        soc_platform = getattr(__import__('soc.platforms').platforms, "{}SocPlatform".format(args.soc))
