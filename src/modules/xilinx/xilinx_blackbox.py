...
class XilinxBlackbox(Elaboratable):
    ...
    def _generate_stub_file(self):
        cls = self.__class__
        while cls.__base__ != XilinxBlackbox:
            cls = cls.__base__
        stub_filename = "{}i".format(inspect.getfile(cls))
        a = self.__class__
            ...
            def gen_class_body(hierarchy_part):
                ...
                def gen_single_child(children, hierarchy_name):
                    ...
                    if isinstance(children, str):
                        ...
                        classes = "\n".join(gen_class("{}_{}".format(hierarchy_name, k), v) for k, v in children.items() if isinstance(v, dict))
                        max_element = max(int(x) for x in children.keys())
                        tuple_inner = ", ".join(
                            "None" if not str(i) in children
                            else to_class_name("{}_{}".format(hierarchy_name, i)) if isinstance(children[str(i)], dict)
                            else "Signal"
