...
from lib.compression.encoding_space import EncodingSpace
...
class ZeroRleDecoder(Elaboratable):
    def __init__(self, input: BasicStream, encoding_space: RleEncodingSpace):
        self.encoding_space = encoding_space

        self.input = input
        # this should work with all kinds of streams so we need to clone it to get the correct output type
        self.output = input.clone()
        self.output.payload = Signal(self.encoding_space.possible_run_lengths_list) @ DOWNWARDS
