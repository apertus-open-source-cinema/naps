...
from util.plot_util import plt_image, plt_show, plt_discrete_hist, plt_hist


def fill_reference_frame(h, w, levels=3):
    ref = np.full((h, w), -1)
    ref[:h // 2, :w // 2] = 0 if levels == 1 else fill_reference_frame(h // 2, w // 2, levels - 1)
...
if __name__ == '__main__':
    ...
    def each(coefficients):
        ...
        if False:
            ...
            plt_image("reference", reference_frame)
            plt_image("reference", color_images[0])
