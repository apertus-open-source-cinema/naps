...
class AxiBufferWriter(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += self.input.ready.eq(input_clone.ready & self.enable)
        m.d.comb += input_clone.valid.eq(self.input.valid & self.enable)

        input_fifo = m.submodules.input_fifo = BufferedSyncStreamFIFO(input_clone, self.fifo_depth)
...
class AddressGenerator(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        with StreamTransformer(self.request, self.output, m):
            ...
            with m.If(~self.request.last & (self.output.payload <= self.max_addrs[self.current_buffer])):
