...
class FT601PerfDebug(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += ClockSignal("ft").eq(ft.clk.i)

        pause_cycles = 0
        in_transaction = Signal()
        pause_counter = Signal(range(pause_cycles + 1))

        with m.If((ft.txe.i & in_transaction) | (ft.txe.i & (pause_counter == pause_cycles))):  # we have space in the transmit fifo
            m.d.ft += self.burst_counter.eq(self.burst_counter + 1)
            m.d.comb += ft.write.o.eq(1)
