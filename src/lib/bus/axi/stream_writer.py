    ...
    def elaborate(self, platform):
        ...
        with m.If(self.address_input.valid & self.data_input.valid & data_output.ready):
            ...
            with m.If((self.address_input.payload == (last_address + self.word_bytes)) & (burst_len < self.max_burst_length - 1)):
                enlarge_burst()
            with m.Elif(self.address_output.ready):
                finish_burst()
        with m.Elif((burst_len > 0) & (timeout_counter == self.burst_creation_timeout - 1) & data_output.ready & self.address_output.ready):
            finish_burst()
