...
class PluginModuleStreamerTx(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        m.submodules.lane_clock = DDRSerializer(0b00001111, self.plugin_resource.clk_word, ddr_domain=self.bitclk_domain)
        m.submodules.lane_valid = DDRSerializer(Repl(valid, 8), self.plugin_resource.valid, ddr_domain=self.bitclk_domain)
        for i in range(4):
            value = Signal(8)
            m.submodules["lane{}".format(i)] = DDRSerializer(value, self.plugin_resource["lane{}".format(i)], ddr_domain=self.bitclk_domain)
            with m.If(valid):
