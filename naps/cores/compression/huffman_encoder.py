...
class HuffmanEncoder(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        code_memory = m.submodules.code_memory = Memory(width=self.max_code_len, depth=self.max_input_word, init=[int(self.table.get(i, '0'), 2) for i in range(self.max_input_word)])
        code_port = code_memory.read_port(domain="sync", transparent=False)
        ...
        m.submodules.code_len_memory = code_len_memory = Memory(width=self.max_code_len, depth=self.max_input_word, init=[len(self.table.get(i, '')) for i in range(self.max_input_word)])
        len_port = code_len_memory.read_port(domain="sync", transparent=False)
