...
class DDRSerializer(Elaboratable):
    def __init__(self, value, pad, ddr_domain, bit_width=8):
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += Cat(oserdes.d[i] for i in reversed(range(1, 9))).eq(self.value ^ self.invert)  # reversed is needed because the order of the Xilinx Oserdes primitives is unintuitive
